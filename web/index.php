<!DOCTYPE html>
<html lang="en">

<head>
   <!-- Required meta tags -->
   <meta charset="utf-8">
   <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">

   <!-- Bootstrap CSS -->
   <link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css" integrity="sha384-ggOyR0iXCbMQv3Xipma34MD+dH/1fQ784/j6cY/iJTQUOhcWr7x9JvoRxT2MZw1T" crossorigin="anonymous">

   <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.min.js" integrity="sha384-JjSmVgyd0p3pXB1rRibZUAYoIIy6OrQ6VrjIEaFf/nJGzIxFDsf4x0xIM+B07jRM" crossorigin="anonymous"></script>

   <title>CS 313 - Assignments</title>
</head>

<body>
   <div class="container">
      <div class="page-header">
         <h1>Uncle Dave's Website</h1>
      </div>
   </div>
   <div class="container">
      <nav class="navbar navbar-expand-sm navbar-light bg-light">
         <div id="navbarSupportedContent">
            <ul class="navbar-nav mr-auto">
               <li class="nav-item active">
                  <a class="nav-link disabled" href="index.php">Assignments</a>
               </li>
               <li class="nav-item">
                  <a class="nav-link" href="about.php">About Me</a>
               </li>
            </ul>
         </div>
      </nav>
      <div class="jumbotron">
         <div class="list-group">
            <a href="week05/add_table.php" class="list-group-item list-group-item-action">
               <div class="d-flex w-100 justify-content-between">
                  <h5 class="mb-1">DB Edit - Week 06 Prove</h5>
                  <small>26 Oct 2019</small>
               </div>
               <p class="mb-1">Add aditional inteface to add, edit, and delete data from the database.</p>
               <small>Percent Complete: 100% <span class="badge badge-danger">LATE</span></small>
            </a>
            <a href="week05/05prove.php" class="list-group-item list-group-item-action">
               <div class="d-flex w-100 justify-content-between">
                  <h5 class="mb-1">DB Access - Week 05 Prove</h5>
                  <small>19 Oct 2019</small>
               </div>
               <p class="mb-1">Build UI for database access.</p>
               <small>Percent Complete: 100%</small>
            </a>
            <a href="http://whispering-thicket-79511.herokuapp.com/liveshare.php" class="list-group-item list-group-item-action">
               <div class="d-flex w-100 justify-content-between">
                  <h5 class="mb-1">DB Access - Week 05 Teach</h5>
                  <small>17 Oct 2019</small>
               </div>
               <p class="mb-1">Make a webpage that displays some scriptures from the database.</p>
               <small>Percent Complete: 100%</small>
            </a>
            <a href="https://github.com/FiscalMonkey/cs313-php/tree/master/db" class="list-group-item list-group-item-action">
               <div class="d-flex w-100 justify-content-between">
                  <h5 class="mb-1">DB Setup - Week 04 Prove</h5>
                  <small>12 Oct 2019</small>
               </div>
               <p class="mb-1">Setup a simple database.</p>
               <small>Percent Complete: 100%</small>
            </a>
            <a href="week03/03prove.php" class="list-group-item list-group-item-action">
               <div class="d-flex w-100 justify-content-between">
                  <h5 class="mb-1">Shopping Cart - Week 03 Prove</h5>
                  <small>5 Oct 2019</small>
               </div>
               <p class="mb-1">Create a simple PHP application to simulate a shopping cart.</p>
               <small>Percent Complete: 93% <span class="badge badge-danger">LATE</span></small>
            </a>
            <a href="week03/03teach.html" class="list-group-item list-group-item-action">
               <div class="d-flex w-100 justify-content-between">
                  <h5 class="mb-1">PHP Forms - Week 03 Teach</h5>
                  <small>2 Oct 2019</small>
               </div>
               <p class="mb-1">Create a form for students to enter information and submit said form using POST.</p>
               <small>Percent Complete: 93%</small>
            </a>
            <a href="week02/02teach.html" class="list-group-item list-group-item-action">
               <div class="d-flex w-100 justify-content-between">
                  <h5 class="mb-1">Week 02 Teach</h5>
                  <small>25 Sep 2019</small>
               </div>
               <p class="mb-1">Super simple webpage that does some basic changes with user interaction.</p>
               <small>Percent Complete: 97.67%</small>
            </a>
         </div>
      </div>
   </div>

   <!-- Using PHP to include the same footer -->
   <?php include("week02/footer.html"); ?>

   <!-- Optional JavaScript -->
   <!-- jQuery first, then Popper.js, then Bootstrap JS -->
   <script src="https://code.jquery.com/jquery-3.3.1.slim.min.js" integrity="sha384-q8i/X+965DzO0rT7abK41JStQIAqVgRVzpbzo5smXKp4YfRvH+8abtTE1Pi6jizo" crossorigin="anonymous"></script>
   <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.7/umd/popper.min.js" integrity="sha384-UO2eT0CpHqdSJQ6hJty5KVphtPhzWj9WO1clHTMGa3JDZwrnQq4sF86dIHNDz0W1" crossorigin="anonymous"></script>
   <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.min.js" integrity="sha384-JjSmVgyd0p3pXB1rRibZUAYoIIy6OrQ6VrjIEaFf/nJGzIxFDsf4x0xIM+B07jRM" crossorigin="anonymous"></script>
</body>

</html>